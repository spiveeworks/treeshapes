...
\begin{enumerate}
	\item Trees have many convenient representations that are invariant under relabelling and many that are able to distinguish trees that have different shape, but not many with both
	\item Matrix spectra are a promising way of removing permutations from a matrix without reducing the matrix to a single number
	\item If a tree representation has an exchange property for some pair of trees, then the representation will be near trivial for large trees.
\end{enumerate}
...
In Matsen it was shown that if a pair of trees exchange then the function for which they exchange will be near-trivial.  % was it?
